			...
			<div class="panel-heading">
				<div class="row">
					<div class="col col-xs-6">
						<h3 class="panel-title">List of JoiningPerks</h3>
					</div>
					<div class="col col-xs-6 text-right">
						<a href="<c:url value="/admin/newJoiningPerk/${cardId}" />"
				...
				<table class="table table-striped table-bordered table-list"
					...
					<tbody>
						<c:forEach items="${joiningPerks}" var="joiningPerk">
							<tr>
								<td class="hidden-xs">${joiningPerk.id}</td>
								<td>${joiningPerk.title}</td>
								<td width="50%">${joiningPerk.description}</td>
								<td align="center"><form
										action="<c:url value="/admin/deleteJoiningPerk/${joiningPerk.id}/${cardId}" />">
										<a href="<c:url value="/admin/updateJoiningPerk/${joiningPerk.id}/${cardId}" />"
